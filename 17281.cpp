...
int hit(vector<bool>& state, int n) {
    ...
    for (int i = 3; i > 0; i--) {
        ...
        if (i + n >= 4) { //홈으로 들어오는 경우
            ...
            state[i + n] = true; //n만큼 이동한 자리가 다음 위치로 업데이트
        }
    }
    //현재 타자의 위치 설정하기
...
void game(int cnt, int n, vector<vector<int>>& inning, int& maxScore) {
    if (cnt == 4) { //4번째 타자는 1로 고정
        game(cnt + 1, n, inning, maxScore); //메인에서 정했으므로 바로 다음으로 넘어가기
    }
    else if (cnt == 10) { //9명의 타자 순서 정해진 경우
        int score = getScore(inning, n); //점수계산
        if (score > maxScore) { //계산한 점수 비교해서 최대값 업데이트하기
            maxScore = score;
        }
    }
    else {
